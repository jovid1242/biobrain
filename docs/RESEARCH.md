...
Сводка обзорного прохода (2026-09-17; дословные цитаты и URL в research-заметках; DOI BANC и male CNS
перепроверены через Crossref). «Синапсы» у разных проектов считаются по-разному: FlyWire — пары pre→post,
Janelia — T-bars (пресинапсы) и PSD (постсинапсы). Прямое сравнение чисел между строками некорректно.

| dataset (версия, дата) | особь | охват | нейроны | синапсы (что считается) | NT | граф без логина | экспорт графа | лицензия | публикация |
|---|---|---|---:|---|---|---|---|---|---|
| **FlyWire FAFB 783** (2023-10; Zenodo 783.0) | самка, 7 дней | мозг + optic lobes | 139,255 | ~130 M пар pre→post; 54.5 M между нейронами | 6 классов | да (Zenodo) | 852 MB feather | CC BY / CC BY-NC | doi:10.1038/s41586-024-07558-y |
| hemibrain v1.2.1 (2021-06) | самка, 5 дней | ~36 % neuropil, обрезано | ~25k | 9.50 M T-bar / 64.1 M PSD | 6 классов | да | 46 MB CSV | противоречиво | doi:10.7554/eLife.57443 |
| optic lobe v1.1 (2025-03) | самец (та же особь, что male CNS) | правая optic lobe | ~53k | 7.34 M pre / 49.5 M post | 7 классов | да | 153 MB feather | CC BY | doi:10.1038/s41586-025-08746-0 |
| **male CNS v1.0** (2026-06) | самец, 5 дней | вся ЦНС | 166,700 | 45.7 M T-bar / 311.8 M PSD | 7 классов | да (GCS, neuPrint) | 508 MB рёбер | CC BY | Cell 2026, doi:10.1016/j.cell.2026.08.015 |
| MANC v1.2.3 (2024-08) | самец, другая особь | VNC | 23,610 | 10.3 M T-bar / 74.5 M PSD | 3 класса | да (v1.0 CSV) | 75 MB | CC BY | doi:10.7554/eLife.97769.1 |
| FANC (публичный дамп CAVE v1444, 2025-10) | самка, трансгенная | VNC, часть обрезана | 14,621 | 45.1 M связей | не найдено | дамп — да, CAVE — ограничен | 12.5 MB parquet | не проверено | doi:10.1038/s41586-024-07389-x |
| **BANC v888** (Nature, 2026-06-08) | самка, дикий тип | вся ЦНС (мозг + VNC) | 155,916 | 218.5 M связей; обе стороны определены у 18 % | 8 классов | да (Dataverse, GCS) | 305 MB feather | CC BY 4.0 | doi:10.1038/s41586-026-10735-w |
| личинка (Winding 2023) | личинка 1-го возраста | мозг + входы | 3,016 | ~548k синаптических сайтов | нет | да | 1.3 MB | CC BY | doi:10.1126/science.add9330 |

Female CNS Janelia объявлена (2025-10-06), но данных нет. Снимка FAFB после 783 нет.

**Вывод обзора: FAFB 783 остаётся лучшей стартовой точкой, но с небольшим отрывом.** Это единственный
весь мозг с неизменяемой DOI-записью и md5, скачиваемой без логина. У него лучшая прикреплённость синапсов:
93.7 % / 44.7 % против 74 % / 23 % у BANC. Предсказания нейромедиатора встроены в таблицу связей, а таблицы
соответствия типов других датасетов привязаны именно к 783.
- **male CNS v1.0** — ближайшая альтернатива: CC BY, вся ЦНС, лучше модель нейромедиаторов. Минусы: нет DOI
  данных, снимок в bucket отличается от live neuPrint, варианты файлов рёбер не документированы.
- **Репликация на второй особи:** (1) male CNS v1.0 через `flywireType` и официальные таблицы сравнения рёбер
  `flyconnectome/2025malecns`; пол и особь при этом смешаны. (2) BANC v888 как проверка на самке через
  `banc_fafb_reviewed_matches`. (3) hemibrain — только на уровне типов.
...
| счёт по super class в статье | суммируется в 127,864, а не в 139,255 | **проверено на аннотациях v2.1.0:** фраза перечисляет только незрительные сенсорные нейроны (5,512); зрительных сенсорных (cell_class `visual`) 11,391 = 139,255 − 127,864. Все остальные классы совпадают точно |
| число cell types | 8,453 (статья) / 5,634 значений `cell_type` в v2.1.0 | 8,453 получается ровно, если считать `cell_type`, а при его отсутствии `hemibrain_type`; покрытие по этому правилу 94.65 % против 96.4 % в статье — знаменатель статьи не ясен |
| аннотации v3.1.0 | 139,248 строк | 7 root id отсутствуют в списке 783, у 14 нейронов из 783 нет строки (1 CX, 3 JO-B, 10 оптических по v2.1.0) |
